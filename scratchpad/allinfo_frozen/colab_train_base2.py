...
PRETEXT_META = os.environ.get("PRETEXT_META", "blank")  # blank=session_meta 제거(빈 META 상수) / omit=[META]라인 생략(A/B 대조군) / keep=데이터 그대로(A/B 실험군: exact 필드) / stable=ALL-INFO Stage A: 복원가능·세션안정 5필드(tier·langpref·turn·loc·langmix)만 [META]에 입력, sentinel 대체 금지
...
assert PRETEXT_META in ("blank", "omit", "keep", "stable")
...
def build_transcript(sample):
    ...
    if PRETEXT_META != "omit":  # omit = [META] 라인 자체 생략 (PRETEXT 전용 — 배포 경로에선 항상 포함)
        ...
        if PRETEXT_META == "stable":  # ALL-INFO Stage A: 복원가능·세션안정 필드만. 없는 키는 생략(0/None/빈문자열 sentinel 대체 금지). langmix 키 정렬.
            lm = ws.get("language_mix") or {}
            lm_str = ",".join(f"{k}:{lm[k]}" for k in sorted(lm))
            _st = []
            if "user_tier" in meta: _st.append(f"tier={_s(meta.get('user_tier'))}")
            if "language_pref" in meta: _st.append(f"langpref={_s(meta.get('language_pref'))}")
            if "turn_index" in meta: _st.append(f"turn={meta.get('turn_index')}")
            if "loc" in ws: _st.append(f"loc={ws.get('loc')}")
            if lm: _st.append(f"langmix={lm_str}")
            parts.append("[META] " + " ".join(_st))
        elif META_TRANS_EXT:  # META-T: 전체 공식 메타를 [META] transcript 에 추가(숫자메타는 불변)
            lm = ws.get("language_mix") or {}
            lm_str = ",".join(f"{k}:{lm[k]}" for k in sorted(lm))
            parts.append(
