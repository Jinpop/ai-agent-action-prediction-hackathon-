...
import os
MINT_PATH=os.environ.get("MINT_PATH","open/data/train_mint_full.jsonl")
mint=feat.load_jsonl(MINT_PATH); mlab=load_labels(MINT_PATH.replace(".jsonl","_labels.csv"))
print(f"[MINT] {MINT_PATH} {len(mint)}행",flush=True)
...
def run(meta_cols,tag):
    ...
    return f1
f_base=run(META128,"128")
print(f"\n[RESULT] {MINT_PATH}: {f_base:.4f}",flush=True)
