...
M = {
 ...
 "mint2b_s81":("scratchpad/hp_mint2b_s81.npy","scratchpad/hidx.npy"),
}
LOG={}; IDX={}
for k,(pp,ip) in M.items():
    LOG[k]=np.load(pp); IDX[k]=np.load(ip)
    assert LOG[k].shape==(14000,14) and IDX[k].shape==(14000,), f"{k} shape"
...
# --- ALIGNMENT VERIFY: single-member holdout F1 vs reported (uses each member's own idx order) ---
print("\n[ALIGN] single-member holdout Macro-F1 (softmax->argmax vs labels at member idx):")
ref = {"k79":0.7617,"k81":0.7567,"mint2b_s79":0.7650,"mint2b_s81":0.7534}
for k in ["k79","k81","mint2b_s79","mint2b_s81"]:
...
# idx equality check: are mint2b/k79/k81 all on identical hidx order?
print("\n[ALIGN] idx-set / idx-order identity:")
for k in ["k79","k81","mint2b_s79","mint2b_s81"]:
...
BASE=["s48","k79","k81"]
combos={"cand_s79 (k81->mint2b_s79)":["s48","k79","mint2b_s79"],
        "cand_s81 (k81->mint2b_s81)":["s48","k79","mint2b_s81"]}

# sanity: v58-core full-blend on full hidx (14000)
inter_full=inter_of(BASE)
bf_plain,bf_se=blend_f1(BASE,inter_full)
print(f"\n[SANITY] v58-core(s48+k79+k81) full-blend on hidx n={len(inter_full)}: plain={bf_plain:.4f} sess-eq={bf_se:.4f} (abc_band ref 0.7702)")

print("\n=== mint2b k81-SLOT BAND CHECK (deploy blend 0.4*classic + 0.6*mean(3TF softmax)) ===")
print(f"{'combo':28s} {'n_rows':>6s} {'n_sess':>6s} {'base_plain':>10s} {'cand_plain':>10s} {'gate(-.0058)':>12s} {'inband(pl)':>10s} | {'base_se':>7s} {'cand_se':>7s} {'gate_se':>8s} {'inband(se)':>10s}")
for name,tf in combos.items():
    ...
    print(f"{name:28s} {len(inter):6d} {nsess:6d} {bp:10.4f} {cp:10.4f} {gate_p:12.4f} {ib_p:>10s} | {bs:7.4f} {cs:7.4f} {gate_s:8.4f} {ib_s:>10s}")
