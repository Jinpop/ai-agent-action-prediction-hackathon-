#!/usr/bin/env python3
"""Holdout A/B: mint_full+relv2(140 meta) vs known baseline mint_full(128)=0.6134.
Same split/recipe as refit_eval2 (champion-exact params, balanced weights)."""
...
va=np.load("scratchpad/hidx.npy")
_va_set=set(va.tolist())
tr0=np.array([i for i in range(len(real_ids)) if i not in _va_set])
...
def run(meta_cols,tag):
    ...
    np.save("scratchpad/hp_classic.npy", blend/blend.sum(1,keepdims=True))
    return f1
f_base=run(META128,"128-hidx")
