...
**서버 실측으로 확립한 법칙:**
1. **정보축은 전달, 정규화축은 반납** — 컨텍스트·데이터·refit·강멤버는 서버에서 이득. FGM·증류·smoothing·스태킹은 홀드아웃에서만 좋아 보이고 서버에서 반납.
2. **다양성 = 최대 레버**(1→3멤버 +0.053), 단 **실체는 레시피 다양성** — 같은 레시피 시드추가(−0.0009)도, 아키텍처만 다른 백본(−0.0040)도, 레시피가 다른 멤버를 못 이김.
3. **홀드아웃 ≠ 서버 서열** — 홀드아웃은 제출 자격 필터일 뿐, 우열 판단은 LB로만.
4. **시간벽이 구조 상한** — 3멤버 598~600s/600s. 4번째 멤버 물리적 불가 → 교체·시간중립 개선만 유효.
5. 시드 draw 스프레드 ≈ 1.5e-5, 블렌드 가중 0.62 argmax, 균등 시드평균 근사최적 — 미세 레버 전부 실측 소진 후 종료.
...
단일 블로킹 SSH로 기다리다 연결이 조용히 끊겨 **완주를 8시간 놓친 사고** 이후 확립:
